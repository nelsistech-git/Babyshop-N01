...
class FwDistributionOrder(models.Model):
    _inherit = 'fw.distribution.order'
    ...
    def _compute_invoice_count(self):
        for rec in self:
            rec.invoice_count = self.env['account.move'].search_count([
                ('fw_distribution_order_id', '=', rec.id),
            ])
    ...
    def action_view_invoices(self):
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'domain': [('fw_distribution_order_id', '=', self.id)],
        }

    def action_create_invoice(self):
        self.ensure_one()
        partner = self.channel_partner_id.partner_id
        if not partner:
            raise UserError(
                "'%s' has no linked Contact. Please set one on the Channel Partner "
                "master before creating an invoice." % self.channel_partner_id.name)
        ...
        move = self.env['account.move'].create({
            ...
            'fw_distribution_order_id': self.id,
