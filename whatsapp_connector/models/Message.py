...
class AcruxChatMessages(models.Model):
    ...
    def message_parse(self):
        ...
        '''
            Messenger tagging: RESPONSE inside standard window vs MESSAGE_TAG outside (policy applies).
            https://developers.facebook.com/docs/messenger-platform/send-messages/message-tags
        '''
        if self.connector_id.is_instagram():
            ...
            if message['type'] in ('image', 'video') and message.get('text', ''):
                raise ValidationError(_('Text in this message is not supported.'))
        elif self.connector_id.is_facebook():
            fb = self.connector_id.facebook_get_outbound_messaging_params(self.contact_id)
            message['messaging_type'] = fb['messaging_type']
            if fb['tag']:
                message['message_tag'] = fb['tag']
            if message['type'] not in ('text', 'image', 'video', 'audio', 'file'):
                raise ValidationError(_('Message type is not supported.'))
        return message

    def _facebook_buttons_to_payload(self, message):
        '''Messenger quick replies vs generic template buttons — see constraints on mixing.'''
        self.ensure_one()
        if self.ttype in ('location', 'contact'):
            raise ValidationError(_('Messenger buttons are not supported on location/contact messages.'))
        if self.chat_list_id:
            raise ValidationError(_('Messenger does not map list messages yet; remove the list.'))
        quick = []
        shell = []
        for btn in self.button_ids:
            if btn.ttype == 'replay':
                quick.append({
                    'content_type': 'text',
                    'title': (btn.text or '')[:20],
                    'payload': (btn.btn_id or btn.text or 'reply')[:1000],
                })
            elif btn.ttype == 'url':
                if not btn.url:
                    raise ValidationError(_('Messenger URL buttons require a valid URL.'))
                shell.append({
                    'type': 'web_url',
                    'url': btn.url.strip()[:1999],
                    'title': (btn.text or '')[:20],
                })
            elif btn.ttype == 'call':
                ph = (btn.phone or '').strip()
                if not ph:
                    raise ValidationError(_('Messenger call buttons require a phone number.'))
                shell.append({
                    'type': 'phone_number',
                    'title': (btn.text or '')[:20],
                    'payload': ph[:128],
        ...
        if quick and shell:
            raise ValidationError(
                _('Mixing Quick Reply and URL/Phone buttons is not supported. Send them in separate messages.'))
        if len(quick) > 13:
            raise ValidationError(_('Messenger allows at most 13 quick replies.'))
        if len(shell) > 3:
            raise ValidationError(_('Messenger generic templates allow at most 3 URL/Call buttons.'))
        if shell and self.ttype != 'text':
            raise ValidationError(
                _('URL and Call Messenger buttons must be sent from a plain text message (attachments: send separately).'))
        if shell:
            body = ((self.text or '').strip())
            lines = body.split('\n')
            title = (lines[0][:80]) if lines and lines[0].strip() else _('Message')
            subtitle = ''
            if len(lines) > 1:
                subtitle = '\n'.join(lines[1:]).strip()[:256]
            elif body and lines[0] and len(lines[0]) > 80:
                subtitle = body.strip()[:256]
            if subtitle[:80] == title[:80]:
                subtitle = ''
            element = {'title': title, 'buttons': shell}
            if subtitle:
                element['subtitle'] = subtitle[:80]
            message['fb_generic_template'] = {
                'template_type': 'generic',
                'elements': [element],
            }
        elif quick:
            message['fb_quick_replies'] = quick
    ...
    def set_buttons(self, message):
        ...
        if self.connector_id.is_facebook():
            self._facebook_buttons_to_payload(message)
            return
    ...
    def _constrains_button_ids(self):
        for message in self:
            if message.button_ids and message.connector_id and message.ttype:
                if message.connector_id.is_facebook():
                    if (not message.ttype.startswith('info')
                            and message.ttype not in {'text', 'image', 'video', 'file', 'audio'}):
                        raise ValidationError(_('Messenger buttons are not supported for type %s') % message.ttype)
                elif message.connector_id.uses_whatsapp_web_api():
