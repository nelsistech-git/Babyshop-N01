...
class BankBookWizard(models.TransientModel):
    ...
    date_to = fields.Date(string='End Date', default=date.today(),
                          required=True)
    set_account = fields.Boolean(string='Set Accounts')
    ...
    @api.onchange('set_account')
    def onchange_set_account_ids(self):
        accounts = []
        journals = self.env['account.journal'].search([('type', '=', 'bank')])
        for journal in journals:
            accounts.append(journal.default_account_id.id)
        if self.set_account:
            return {'domain': {'account_ids': [('id', 'in', accounts)]}, 'value': {'account_ids': accounts}}
        else:
            return {'domain': {'account_ids': [('id', 'in', accounts)]}, 'value': {'account_ids': None}}
