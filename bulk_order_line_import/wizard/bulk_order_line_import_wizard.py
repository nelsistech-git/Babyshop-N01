...
COLUMN_ALIASES = {
    ...
    'uom': [
        'uom', 'unitofmeasure', 'unit', 'productuom',
    ],
    'discount': [
...
class BulkOrderLineImportWizard(models.TransientModel):
    _name = 'wizard.bulk.order.line.import'
    _description = 'Bulk Sales/Purchase Order Line Import Wizard'

    res_model = fields.Selection([
        ('sale.order', 'Sales Order'),
        ('purchase.order', 'Purchase Order'),
    ], string='Order Type', required=True)
    res_id = fields.Integer(string='Order', required=True)
    order_reference = fields.Char(compute='_compute_order_reference')
    ...
    duplicate_strategy = fields.Selection([
        ...
        help='Separate: every row in the file becomes its own order line, '
             'even if the same product appears more than once.\n'
             'Merge: quantities for the same product code within this file '
             'are summed into a single order line.')

    @api.depends('res_model', 'res_id')
    def _compute_order_reference(self):
        for rec in self:
            rec.order_reference = False
            if rec.res_model and rec.res_id:
                order = self.env[rec.res_model].browse(rec.res_id)
                if order.exists():
                    rec.order_reference = order.display_name
    ...
    def action_download_sample_template(self):
        ...
        headers = ['Product Code / SKU', 'Quantity', 'Unit Price', 'UoM', 'Discount (%)']
        sheet.append(headers)
        for col in range(1, len(headers) + 1):
            sheet.cell(row=1, column=col).font = openpyxl.styles.Font(bold=True)

        is_sale = self.res_model == 'sale.order'
        sheet.append(['DEMO-0001', 5, '', 'Units', 10 if is_sale else ''])
        sheet.append(['DEMO-0002', 2.5, 99.90, '', ''])

        notes = sheet.cell(row=5, column=1, value=(
            'Product Code / SKU and Quantity are required. Unit Price, UoM'
            + (' and Discount (%)' if is_sale else '')
            + ' are optional - leave blank to let Odoo compute them '
              'automatically from the product and pricelist.'
        ))
        ...
        attachment = self.env['ir.attachment'].create({
            'name': 'bulk_order_line_import_template.xlsx',
            'type': 'binary',
            'datas': content,
    ...
    def _read_rows(self):
        ...
        if ext == 'csv':
            ...
            # Sniff delimiter (comma vs semicolon) with a safe fallback.
            try:
    ...
    def action_import(self):
        ...
        if not order.exists():
            raise UserError(_('The order this wizard was opened from no longer exists.'))
        if order.state not in ('draft', 'sent'):
            raise UserError(_(
                'Lines can only be imported while the order is in Draft or Sent state.'))
        ...
        for i, row in enumerate(data_rows, start=2):  # row 1 is the header
            ...
            sku = str(cell('sku')).strip()
            if isinstance(cell('sku'), float) and cell('sku').is_integer():
                # Excel stores "1001" as 1001.0 - avoid "1001.0" mismatches.
                sku = str(int(cell('sku')))
            if not sku:
            ...
            parsed.append({
                ...
                'sku': sku,
                'qty_raw': cell('qty'),
                'price_raw': cell('price') if 'price' in col_map else '',
                'uom_raw': str(cell('uom')).strip() if 'uom' in col_map else '',
                'discount_raw': cell('discount') if 'discount' in col_map else '',
            })
        ...
        for p in parsed:
            ...
            if price_raw not in ('', None):
                try:
                    ...
                    errors.append(_(
                        "Row %(row)s: Invalid unit price '%(val)s'."
                    ) % {'row': p['row'], 'val': price_raw})
                    continue

            discount = 0.0
            if is_sale and p['discount_raw'] not in ('', None):
                try:
                    discount = float(str(p['discount_raw']).replace(',', '').replace('%', '').strip())
                except (TypeError, ValueError):
                    errors.append(_(
                        "Row %(row)s: Invalid discount '%(val)s'. Discount must be a "
                        "numeric percentage between 0 and 100."
                    ) % {'row': p['row'], 'val': p['discount_raw']})
                    continue
                if not (0 <= discount <= 100):
                    errors.append(_(
                        "Row %(row)s: Invalid discount '%(val)s'. Discount must be "
                        "between 0 and 100."
                    ) % {'row': p['row'], 'val': p['discount_raw']})
                    continue
            ...
            if p['uom_raw']:
                ...
                # own unit, matching the SRS fallback rule.
        ...
        try:
            with self.env.cr.savepoint():
                if is_sale:
                    self._create_sale_lines(order, resolved)
                else:
                    self._create_purchase_lines(order, resolved)
        except UserError:
            raise
    ...
    def _create_sale_lines(self, order, resolved):
        vals_list = []
        for r in resolved:
            vals = {
                'order_id': order.id,
            ...
            # product_uom is precomputed from the product when omitted, so
            # only set it explicitly when the file specified a valid UoM.
            if r['uom']:
                vals['product_uom'] = r['uom'].id
            if r['price'] is not None:
                vals['price_unit'] = r['price']
            if r['discount']:
                vals['discount'] = r['discount']
            vals_list.append(vals)
        # A single batched create() lets the ORM group computations and SQL
        # inserts, which is an order of magnitude faster than creating
        # lines one at a time for large files.
        self.env['sale.order.line'].create(vals_list)

    def _create_purchase_lines(self, order, resolved):
        vals_list = []
        for r in resolved:
            product = r['product']
            # purchase.order.line.product_uom has no automatic default on
            # create() (unlike the sale order line), so replicate the same
            # fallback the standard product_id onchange uses.
            uom = r['uom'] or product.uom_po_id or product.uom_id
            vals = {
                'order_id': order.id,
