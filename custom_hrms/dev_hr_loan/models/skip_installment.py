...
class DevSkipInstallment(models.Model):
    ...
    skip_to = fields.Selection([('0', 'Next Month'),
                              ('1', 'New Month')
                              ], string='Skip To', default='1')
    ...
    def action_send_request(self):
        ...
        if self.manager_id and self.manager_id.work_email:
            ...
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.dev_skip_dep_manager_approval')
    ...
    def approve_skip_installment(self):
        ...
        if email:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.dev_skip_ins_hr_manager_request')
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id[1])
    ...
    def dep_reject_skip_installment(self):
        if self.employee_id.work_email:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.dep_manager_reject_skip_installment')
    ...
    def hr_reject_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.hr_manager_reject_skip_installment')
    ...
    def confirm_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.hr_manager_confirm_skip_installment')
    ...
    def create_skip_account_journal_entry(self):
        if not self.employee_id.address_home_id:
            # if not self.employee_id.p_address_id:
            raise ValidationError(_('Employee Private Address is not selected in Employee Form !!!'))
        else:
            partner_id = self.employee_id.address_home_id.id

        if self.loan_id.loan_type_id.journal_id and self.loan_id.loan_type_id.journal_id.is_pf_display:
            fs_dept = 'pf'
        else:
            fs_dept = 'accounts'
        vals = {
            ...
            'location_id': self.employee_id.user_work_location_id.id,
            'fs_dept': fs_dept
        }
    ...
    def done_skip_installment(self):
        ...
        if skip_to == '0':
            ...
            date = max(self.loan_id.installment_lines.mapped('date'))
        else:
            raise ValidationError(_('Required Skip To !!!'))
