...
class AttendanceDevice(models.Model):
    ...
    def _get_default_attendance_states(self):
        ...
        attendance_device_state_code_0 = self.env.ref('custom_zk_attendance_device.attendance_device_state_code_0')
        if attendance_device_state_code_0:
            att_state_ids += attendance_device_state_code_0
        attendance_device_state_code_1 = self.env.ref('custom_zk_attendance_device.attendance_device_state_code_1')
        if attendance_device_state_code_1:
            att_state_ids += attendance_device_state_code_1

        # Overtime Working Attendance
        attendance_device_state_code_4 = self.env.ref('custom_zk_attendance_device.attendance_device_state_code_4')
        if attendance_device_state_code_4:
            att_state_ids += attendance_device_state_code_4
        attendance_device_state_code_5 = self.env.ref('custom_zk_attendance_device.attendance_device_state_code_5')
        if attendance_device_state_code_5:
            att_state_ids += attendance_device_state_code_5
        # attendance_device_state_code_255 = self.env.ref('custom_zk_attendance_device.attendance_device_state_code_255')
    ...
    location_id = fields.Many2one('attendance.device.location', string='Location', tracking=True,
                                  help='The location where the device is located')
    ...
    is_allow_mobile_app = fields.Boolean(string='Mobile App used?', default=False, help='Mobile app attendance users will use this device.')
    ...
    def connect(self):
        ...
        def post_message():
            email_template_id = self.env.ref('custom_zk_attendance_device.email_template_attendance_device')
    ...
    def getAttendance(self):
        ...
        try:
            ...
            if post_err_msg and self.download_error_notification:
                email_template_id = self.env.ref('custom_zk_attendance_device.email_template_error_get_attendance')
                self.post_message(email_template_id)
            self.enableDevice()
            self.disconnect()
    ...
    def _download_users_by_uid(self):
        ...
        for r in self:
            ...
            device_user_ids = DeviceUser.with_context(active_test=False).sudo().search([('device_id', '=', r.id)])
            ...
            for device_user in device_users:
                ...
                if uid not in existing_user_ids:
                    ...
                    existing = DeviceUser.with_context(active_test=False).sudo().search([('uid', '=', uid), ('device_id', '=', r.id)], limit=1)
                    if existing:
                        update_data = {}
        ...
        for r in self:
            ...
            for device_user in device_users:
                ...
                if user_id not in existing_user_ids:
                    ...
                    existing = DeviceUser.with_context(active_test=False).sudo().search([
                        ('user_id', '=', user_id),
    ...
    def action_attendance_download(self):
        ...
        for r in self:
            ...
            for attendance in attendance_data:
                ...
                user_id = AttendanceUser.with_context(active_test=False).sudo().search([
    ...
    def action_finger_template_download(self):
        ...
        for r in self:
            r.action_employee_map()
            device_users = self.env['attendance.device.user'].sudo().search([('device_id', '=', r.id)])
            ...
            finger_template_ids = FingerTemplate.sudo().search([('device_id', '=', r.id)])
            ...
            for template in template_data:
                ...
                if (str(template.uid) + '_' + str(template.fid)) not in existing_finger_template_ids:
                    ...
                    existing = FingerTemplate.sudo().search([
                        ('uid', '=', uid),
                        ('fid', '=', fid),
                        ('device_id', '=', r.id),
                        ], limit=1)
    ...
    def is_attendance_clear_safe(self):
        ...
        for att in attendances:
            if att.punch not in check_statuses:
                continue
            user = User.with_context(active_test=False).sudo().search([('user_id', '=', att.user_id), ('device_id', '=', self.id)], limit=1)
            utc_dt = self.convert_time_to_utc(att.timestamp, self.tz)
            str_utc_dt = fields.Datetime.to_string(utc_dt)
            match = UserAttendance.sudo().search([('device_id', '=', self.id),
                                           ('user_id', '=', user.id),
                                           ('status', '=', att.punch),
        ...
        for r in self:
            ...
            if attendance_clear_safe:
                ...
                if r.auto_clear_attendance_error_notif:
                    email_template_id = self.env.ref('custom_zk_attendance_device.email_template_not_safe_to_clear_attendance')
    ...
    def action_view_users(self):
        action = self.env.ref('custom_zk_attendance_device.device_user_list_action')
        ...
        if self.device_users_count != 1:
            ...
            res = self.env.ref('custom_zk_attendance_device.attendance_device_user_form_view', False)
    ...
    def action_view_attendance_data(self):
        self.ensure_one()
        action = self.env.ref('custom_zk_attendance_device.action_user_attendance_data')
        result = action.read()[0]
        ...
        if total_att_records != 1:
            ...
            res = self.env.ref('custom_zk_attendance_device.view_attendance_data_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_mapped_employees(self):
        ...
        if self.mapped_employees_count != 1:
            ...
            res = self.env.ref('custom_zk_attendance_device.view_employee_form', False)
            result['views'] = [(res and res.id or False, 'form')]
