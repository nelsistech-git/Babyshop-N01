...
class AttendanceDeviceUser(models.Model):
    ...
    user_id = fields.Char(string='ID Number', help='The ID Number of the user/employee in the device storage', required=True, tracking=True)
    ...
    def smart_find_employee(self):
        ...
        if self.employee_id:
            ...
            for employee in self.device_id.unmapped_employee_ids:
                if self.user_id == employee.device_user_id:
