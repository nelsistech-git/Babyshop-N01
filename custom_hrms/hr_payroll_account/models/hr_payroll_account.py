...
class HrPayslip(models.Model):
    ...
    salary_payment_id = fields.Many2one('salary.payment', string='Salary Payment',
                                        copy=False,
                                        ondelete='cascade',
    ...
    def _action_create_account_move(self):
        ...
        for journal_id in slip_mapped_data: # For each journal_id.
            for slip_date in slip_mapped_data[journal_id]: # For each month.
                ...
                credit_sum = 0.0
                date = slip_date
                partner_id = self.employee_id.address_home_id.id if self.employee_id.address_home_id else None
    ...
    def _prepare_line_values(self, line, account_id, date, debit, credit):
        return {
            'name': line.name,
            'partner_id': line.slip_id.employee_id.address_home_id.id if line.slip_id.employee_id.address_home_id else None, #line.partner_id.id,
            'account_id': account_id,
            'journal_id': line.slip_id.struct_id.journal_id.id,
            'date': date,
            'debit': debit,
            'credit': credit,
            'analytic_line_ids': [(6, 0, [line.salary_rule_id.analytic_account_id.id or line.slip_id.contract_id.analytic_account_id.id])],
        }
