...
class HrAttendancePolicy(models.Model):
    ...
    salary_ded_count_late = fields.Integer(string="Salary Deduct Count (Late)", default=1)
    salary_ded_count_early_out = fields.Integer(string="Salary Deduct Count (Early Out)", default=1)
    salary_ded_count_abs = fields.Integer(string="Salary Deduct Count (Absence)", default=1)

    work_day_without_week_ph = fields.Boolean(string="Worked Days without Weekend Holiday", default=False)
    work_day_without_ph = fields.Boolean(string="Worked Days without Public Holiday", default=False)
    is_att_join_resign = fields.Boolean(string="Join/Resign count from attendance?", default=True, help="Join/Resign calculation from attendance? otherwise employee Join Date or Separation date.")
    is_per_day_salary_from_workday = fields.Boolean(string="Per Day Salary/OT from working days?", default=True, help="Per Day Salary/OT calculation from working days? otherwise calculation from calendar days.")
    is_per_day_salary_from_basic = fields.Boolean(string="Per Day Salary/OT from Basic?", default=False, help="Per Day Salary/OT calculation from Basic Amount? otherwise calculation from Gross Amount.")

    check_in_start = fields.Float(string="Check-in Start", default=0.00)
    check_in_end = fields.Float(string="Check-in End", default=0.00)
    ot_calc_type = fields.Selection([
        ('fixed', 'Fixed'),
        ('working_days', 'Working Days'),
    ], required=True, default="working_days", string='Overtime Calculation Type')

    is_absent_allowed_att_bonus = fields.Boolean(string="Is Absent Allowed?", default=False)
    absent_count_att_bonus = fields.Float(string="Absent Count", default=0.00)
    is_leave_allowed_att_bonus = fields.Boolean(string="Is Leave Allowed?", default=False)
    leave_count_att_bonus = fields.Float(string="Leave Count", default=0.00)
    is_late_allowed_att_bonus = fields.Boolean(string="Is Late Allowed?", default=False)
    late_count_att_bonus = fields.Float(string="Late Count", default=0.00)
    is_diff_allowed_att_bonus = fields.Boolean(string="Is Early-out Allowed?", default=False)
    diff_count_att_bonus = fields.Float(string="Early-out Count", default=0.00)

    is_tiffin_allowed_absent = fields.Boolean(string="Is Absent Allowed?", default=False)
    is_tiffin_allowed_leave = fields.Boolean(string="Is Leave Allowed?", default=False)
    ...
    def get_late(self, period):
        ...
        if self:
            if self.late_rule_id:
                ...
                time_ids = self.late_rule_id.line_ids.filtered(lambda x: x.late_type == 'common').sorted(
                    key=lambda r: r.time, reverse=True)
    ...
    def get_late_abs(self, period):
        ...
        if self:
            if self.late_rule_id:
                ...
                time_ids = self.late_rule_id.line_ids.filtered(lambda x: x.late_type == 'late_abs').sorted(
                    key=lambda r: r.time, reverse=True)
                for line in time_ids:
                    if period >= line.time:
                        flag = True
...
class HrLateRuleLine(models.Model):
    ...
    late_type = fields.Selection(string="Type", selection=late_type, default='common', required=True)
