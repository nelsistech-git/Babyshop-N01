...
class HrPayrollStructure(models.Model):
    ...
    regular_pay = fields.Boolean("Regular/Salary Pay", help="Check this option if this structure is the common one, Salary structure")
    ...
    def get_all_rules(self):
        ...
        all_rules = []
        for struct in self:
            all_rules += struct.input_line_type_ids
...
class HrPayrollStructureType(models.Model):
    ...
    country_id = fields.Many2one('res.country', string='Country', default=lambda self: self.env.company.country_id)
    wage_type = fields.Selection([('monthly', 'Monthly Wage'), ('hourly', 'Hourly Wage')], default='monthly', required=True)
...
class HrSalaryRule(models.Model):
    ...
    category_code = fields.Char('Category Code', related="category_id.code")
    active = fields.Boolean(default=True,
    ...
    is_other = fields.Boolean(default=False,
        help="If the is_other field is set to True, it will be counted as Other Allowances or Deduction")
    ...
    disbursement_type = fields.Selection([
        ('bank', 'Bank'),
        ('cash', 'Cash')
    ], string="Payment Type (Fixed)", help="if disbursement type fixed, without contract settings")
