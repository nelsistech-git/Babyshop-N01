...
class EmployeeGratuity(models.Model):
    ...
    def _onchange_employee_id(self):
        ...
        if self.employee_id.id:
            ...
            self.employee_joining_date = joining_date = self.employee_id.initial_employment_date
            ...
            # find configuration ids related to the gratuity accounting configuratigon
            self.employee_gratuity_configuration = hr_accounting_configuration_id.id
            eligible_worked_years = hr_accounting_configuration_id.eligible_worked_years
            self.employee_gratuity_years = round(self.employee_gratuity_years - eligible_worked_years, 2)
            ...
            if self.total_working_years < eligible_worked_years and self.employee_id.id:
...
class EmployeeContractWage(models.Model):
    ...
    company_country_id = fields.Many2one('res.country', string="Company country", related='company_id.country_id',
                                         readonly=True)
    wage_type = fields.Selection([('monthly', 'Monthly Fixed Wage'), ('hourly', 'Hourly Wage')])
    hourly_wage = fields.Monetary('Hourly Wage', default=0, required=True,
