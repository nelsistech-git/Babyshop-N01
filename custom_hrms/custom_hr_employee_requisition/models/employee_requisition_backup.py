...
class EmployeeRequisition(models.Model):
    ...
    #     context = dict(self.env.context)  # Clone the current context to preserve other context data
    #     context.update({
    #         'user_id': self.env.user.id,  # Add custom data to the context
    #         'docids': self.ids,  # Ensure docids is correctly passed if not already included
    #     })
    #     # return self.env.ref('custom_hr_employee_requisition.hr_emp_rec_print_report').report_action(self, data=data)
    #     return self.env.ref('custom_hr_employee_requisition.hr_emp_rec_print_report').report_action(self.ids, data=data)
