...
class DailyAttendanceSheetWizard(models.TransientModel):
    _name = "daily.attendance.sheet.wizard"
    _description = "Daily Attendance Sheet Wizard"

    file_data = fields.Binary('Daily Attendance Sheet Wizard')
    ...
    category_ids = fields.Many2many('hr.employee.category', 'daily_attendance_sheet_employee_category_rel', 
    ...
    def daily_attendance_sheet_excel(self):
        ...
        # get data from sql
        #data = self.daily_attendance_sheet_sql(from_date, to_date, user_work_location_id, department_ids, employee_id, status)
        data = self.daily_attendance_sheet_sql()

        from_date = datetime.strptime(str(from_date), '%Y-%m-%d').strftime('%d-%b-%Y')
        to_date = datetime.strptime(str(to_date), '%Y-%m-%d').strftime('%d-%b-%Y')

        file_name = "Daily Attendance Sheet (%s - %s).xlsx" % (data['form']['from_date'], data['form']['to_date'])
        ...
        sheet = workbook.add_worksheet('Daily Attendance Sheet')
        ...
        if employee_id:
            ...
            sheet.merge_range(1, 0, 2, 10, "Daily Attendance Sheet", format0)
            ...
            sheet.merge_range(3, 4, 3, 10, 'Work Location: {0}'.format(employee_id.user_work_location_id.name), format1)
            sheet.merge_range(4, 4, 4, 10, 'Department: {0}'.format(employee_id.department_id.name), format1)
            sheet.merge_range(5, 4, 5, 10, 'Designation: {0}'.format(employee_id.job_id.name), format1)
            sheet.merge_range(6, 4, 6, 10, 'Joining Date: {0}'.format(data['emp_joining_date']), format1)
            sheet.merge_range(7, 4, 7, 10, 'Tags: {0}'.format(','.join(self.category_ids.mapped('display_name'))) if self.sbu_unit_id else "Tags: No Tags Selected", format1)

            sheet.write(8, 0, 'Date', format2)
            sheet.write(8, 1, 'Check-in', format2)
            sheet.write(8, 2, 'Check-out', format2)
            sheet.write(8, 3, 'Work Hour', format2)
            sheet.write(8, 4, 'Overtime', format2)
            sheet.write(8, 5, 'Late-in', format2)
            sheet.write(8, 6, 'Early Out', format2)
            sheet.write(8, 7, 'Auto/Manual', format2)
            sheet.write(8, 8, 'Manual Reason', format2)
            sheet.write(8, 9, 'Status', format2)
            sheet.write(8, 10, 'Note', format2)
            ...
            for rec in data['csr']:
                ...
                if rec['punch_count'] != 1:
                    sheet.write(row, col + new_col,
                                str(timedelta(hours=int(rec['check_out']), minutes=(rec['check_out'] * 60) % 60,
                                                seconds=00)), format5)
                    new_col += 1
                    check_in_hours, check_in_minutes = divmod(rec['check_in'] * 60, 60)
                    check_out_hours, check_out_minutes = divmod(rec['check_out'] * 60, 60)

                    check_in_time = timedelta(hours=int(check_in_hours), minutes=int(check_in_minutes), seconds=0)
                    check_out_time = timedelta(hours=int(check_out_hours), minutes=int(check_out_minutes), seconds=0)
                    time_worked = check_out_time - check_in_time
                    total_time_worked += time_worked
                else:
                    sheet.write(row, col + new_col, None, format5)
                    new_col += 1

                sheet.write(row, col + new_col, str(time_worked), format5)
                new_col += 1
                sheet.write(row, col + new_col,
                ...
                #             datetime.strptime(str(rec['check_in'] + timedelta(hours=6)), '%Y-%m-%d %H:%M:%S').strftime(
                #                 '%d-%b-%Y %H:%M:%S'), format5)
                # sheet.write(row, col + 2,
                #             datetime.strptime(str(rec['check_out'] + timedelta(hours=6)), '%Y-%m-%d %H:%M:%S').strftime(
                #                 '%d-%b-%Y %H:%M:%S'), format5)
                # sheet.write(row, col + 3,
                #             str(timedelta(hours=int(rec['overtime']), minutes=(rec['overtime'] * 60) % 60,
                #                           seconds=00)), format5)
                # sheet.write(row, col + 4, str(timedelta(hours=int(rec['late_in']), minutes=(rec['late_in'] * 60) % 60,
                #                                         seconds=00)), format5)
                # sheet.write(row, col + 5,
                #             str(timedelta(hours=int(rec['diff_time']), minutes=(rec['diff_time'] * 60) % 60,
                #                           seconds=00)), format5)
                sheet.write(row, col + new_col, rec['att_type'], format5)
                new_col += 1
                sheet.write(row, col + new_col, rec['manual_reason'], format5)
                new_col += 1
                sheet.write(row, col + new_col, rec['status'], format5)
                new_col += 1
                sheet.write(row, col + new_col, rec['note'], format5)
...
#identical
            ...
            if not data['form']['status'] or data['form']['status'] == 'present':
                ...
                sheet.write(final_row, final_col + new_col, total_present, format9)
                final_row = final_row + 1
            if not data['form']['status'] or data['form']['status'] == 'ab':
                sheet.write(final_row, final_col, 'Total Absent:', format8)
                sheet.write(final_row, final_col + new_col, total_absent, format9)
                final_row = final_row + 1
            if not data['form']['status'] or data['form']['status'] == 'late_in':
                sheet.write(final_row, final_col, 'Total Late In:', format8)
                sheet.write(final_row, final_col + new_col, total_late_in, format9)
                final_row = final_row + 1
            if not data['form']['status'] or data['form']['status'] == 'early_out':
                sheet.write(final_row, final_col, 'Total Early Out:', format8)
                sheet.write(final_row, final_col + new_col, total_early_out, format9)
                final_row = final_row + 1
            if not data['form']['status'] or data['form']['status'] == 'leave':
                sheet.write(final_row, final_col, 'Total Leave:', format8)
                sheet.write(final_row, final_col + new_col, total_leave, format9)
                final_row = final_row + 1
            sheet.write(final_row, final_col, 'Total Weekend:', format8)
            sheet.write(final_row, final_col + new_col, total_wk, format9)
            final_row = final_row + 1
            sheet.write(final_row, final_col, 'Total Public Holiday:', format8)
            sheet.write(final_row, final_col + new_col, total_ph, format9)
            final_row = final_row + 1

            total_hours = total_time_worked.total_seconds() / 3600
            total_hours_rounded = round(total_hours, 2)
            sheet.write(final_row, final_col, 'Total Work Hours:', format8)
            sheet.write(final_row, final_col + new_col, total_hours_rounded, format9)

# up till now identical

        else:
            sheet.merge_range(0, 0, 0, 15, "{0}".format(data['form']['company_id'][1]), format0)
            sheet.merge_range(1, 0, 2, 15, "Daily Attendance Sheet", format0)
            ...
            sheet.merge_range(3, 6, 3, 15, 'Employee: {0}'.format(data['emp_name']), format1)
            sheet.merge_range(4, 6, 4, 15, 'Department: {0}'.format(data['dept_name']), format1)
            sheet.merge_range(5, 6, 5, 15, 'Status: {0}'.format(data['status_name']), format1)
            sheet.merge_range(6, 6, 6, 15, 'Tags: {0}'.format(','.join(self.category_ids.mapped('display_name'))) if self.sbu_unit_id else "Tags: No Tags Selected", format1)
            # sheet.write(6, 0, 'Date', format2)
            # sheet.write(6, 1, 'Employee', format2)
            # sheet.write(6, 2, 'Employee ID', format2)
            # sheet.write(6, 3, 'Work Location', format2)
            # sheet.write(6, 4, 'Department', format2)
            # sheet.write(6, 5, 'Designation', format2)
            # sheet.write(6, 6, 'Check-in', format2)
            # sheet.write(6, 7, 'Check-out', format2)
            # sheet.write(6, 8, 'Overtime', format2)
            # sheet.write(6, 9, 'Late-in', format2)
            # sheet.write(6, 10, 'Early Out', format2)
            # sheet.write(6, 11, 'Auto/Manual', format2)
            # sheet.write(6, 12, 'Manual Reason', format2)
            # sheet.write(6, 13, 'Status', format2)
            # sheet.write(6, 14, 'Note', format2)
            ...
            for rec in data['csr']:
                ...
                #----------------
                new_dept=str(rec['dept_name'])
                if prev_dept!=new_dept:
                    if prev_dept != '-1':
                        sheet.merge_range(row, 0, row, 15, prev_dept + ' Total: '+ str(dept_emp_count), format1)
                        row +=2
                    sheet.merge_range(row, 0, row, 15, 'Department: ' + rec['dept_name'], format1)
                    row += 1
                    sheet.write(row, 0, 'Date', format2)
                    sheet.write(row, 1, 'Employee', format2)
                    sheet.write(row, 2, 'Employee ID', format2)
                    sheet.write(row, 3, 'Work Location', format2)
                    sheet.write(row, 4, 'Department', format2)
                    sheet.write(row, 5, 'Designation', format2)
                    sheet.write(row, 6, 'Check-in', format2)
                    sheet.write(row, 7, 'Check-out', format2)
                    sheet.write(row, 8, 'Work Hour', format2)
                    sheet.write(row, 9, 'Overtime', format2)
                    sheet.write(row, 10, 'Late-in', format2)
                    sheet.write(row, 11, 'Early Out', format2)
                    sheet.write(row, 12, 'Auto/Manual', format2)
                    sheet.write(row, 13, 'Manual Reason', format2)
                    sheet.write(row, 14, 'Status', format2)
                    sheet.write(row, 15, 'Note', format2)
                    row += 1
                    prev_dept = new_dept
                    dept_emp_count = 0
                ...
                sheet.write(row, col, datetime.strptime(str(rec['date']), '%Y-%m-%d').strftime('%d-%b-%Y'), format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['emp_name'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['id_card_no'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['loc_name'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['dept_name'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['desig_name'], format5)
                sheet.write(row, col + (new_col := new_col + 1), str(timedelta(hours=int(rec['check_in']), minutes=(rec['check_in'] * 60) % 60,
                                                        seconds=00)), format5)
                if rec['punch_count'] != 1:
                    sheet.write(row, col + (new_col := new_col + 1), str(timedelta(hours=int(rec['check_out']), minutes=(rec['check_out'] * 60) % 60, seconds=00)), format5)
                    check_in_hours, check_in_minutes = divmod(rec['check_in'] * 60, 60)
                    check_out_hours, check_out_minutes = divmod(rec['check_out'] * 60, 60)
                    ...
                    sheet.write(row, col + (new_col := new_col + 1), None, format5)

                sheet.write(row, col + (new_col := new_col + 1), str(time_worked), format5)
                sheet.write(row, col + (new_col := new_col + 1), str(timedelta(hours=int(rec['overtime']), minutes=(rec['overtime'] * 60) % 60, seconds=00)), format5)
                sheet.write(row, col + (new_col := new_col + 1), str(timedelta(hours=int(rec['late_in']), minutes=(rec['late_in'] * 60) % 60, seconds=00)), format5)
                sheet.write(row, col + (new_col := new_col + 1), str(timedelta(hours=int(rec['diff_time']), minutes=(rec['diff_time'] * 60) % 60, seconds=00)), format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['att_type'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['manual_reason'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['status'], format5)
                sheet.write(row, col + (new_col := new_col + 1), rec['note'], format5)
            ...
            #end loop
            if int(dept_emp_count) > 0:
                if prev_dept!="-1":
                    sheet.merge_range(row, 0, row, 15, prev_dept + ' Total: ' + str(dept_emp_count), format1)
            #sheet.merge_range(row, 7, row, 14, prev_dept + ' Total Hour: ' + str(total_hours_rounded), format1)
            #----------
            final_row = row + 2
            final_col = 0
            sheet.write(final_row, final_col, 'Total Employee:', format8)
            ...
            final_row = final_row + 1
            sheet.write(final_row, final_col, 'Total Work Hours:', format8)
            sheet.write(final_row, final_col + 1, total_hours_rounded, format9)
        ...
        return {
            ...
            'url': '/web/content?model=daily.attendance.sheet.wizard&field=file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'self',
        }

    def daily_attendance_sheet_pdf(self):
        from_date = self.from_date
        to_date = self.to_date
        user_work_location_id = self.user_work_location_id
        department_ids = self.department_ids
        ...
        if employee_id:
            return self.env.ref(
                'custom_hr_report.daily_attendance_sheet_report_id').with_context(landscape=False).report_action(self,
                                                                                                                data=data)
        else:
            return self.env.ref(
                'custom_hr_report.daily_attendance_sheet_report_id').with_context(landscape=True).report_action(self,
                                                                                                                data=data)

    def daily_attendance_sheet_sql(self):
        from_date = self.from_date
        to_date = self.to_date
        user_work_location_id = self.user_work_location_id
        department_ids = self.department_ids
        ...
        # data_sql = """
        #             SELECT tbl1.tr_date, hre.id AS emp_id, hre.name AS emp_name, hre.id_card_no, tbl1.check_in, tbl1.check_out, tbl1.overtime,
        #             tbl1.att_type, sl.name AS loc_name, hd.name AS dept_name, hj.name AS desig_name, tbl1.late_in, tbl1.diff_time
        #             FROM (
        #                 SELECT tr_date::DATE, hra.employee_id, hra.check_in, hra.check_out, hra.overtime,
        #                 CASE WHEN manual_flag=1 THEN COALESCE('Manual', '') ELSE 'Auto' END AS att_type, hra.late_in, hra.diff_time
        #                 FROM generate_series(
        #                   '{0}'::DATE,
        #                   '{1}'::DATE,
        #                   '1 day'::INTERVAL
        #                 ) tr_date
        #                 LEFT JOIN hr_attendance hra ON hra.attendance_date = tr_date
        #                 WHERE DATE(attendance_date) BETWEEN '{0}' AND '{1}'
        #                 GROUP BY tr_date, hra.employee_id, hra.check_in, hra.check_out, hra.overtime, hra.manual_flag, hra.late_in, hra.diff_time
        #                 ORDER BY tr_date, hra.employee_id
        #             ) tbl1
        ...
        #             ORDER BY tbl1.tr_date, hre.name
        #             """.format(from_date, to_date, work_loc_filter, dept_filter, emp_filter)
        ...
        data_sql = """
                    SELECT main_tbl.date, main_tbl.emp_id, main_tbl.emp_name, main_tbl.joining_date, COALESCE(main_tbl.id_card_no, '') AS id_card_no, COALESCE(main_tbl.loc_name, '') AS loc_name, COALESCE(main_tbl.dept_name, '') AS dept_name, COALESCE(main_tbl.desig_name, '') AS desig_name, COALESCE(main_tbl.ac_sign_in, 0) AS check_in, COALESCE(main_tbl.ac_sign_out, 0) AS check_out, COALESCE(main_tbl.punch_count, 0) AS punch_count,COALESCE(main_tbl.overtime, 0) AS overtime,
                        main_tbl.att_type, COALESCE(main_tbl.late_in, 0) AS late_in, COALESCE(main_tbl.diff_time, 0) AS diff_time, main_tbl.status AS main_status, main_tbl.manual_reason AS manual_reason,main_tbl.note AS note,
                        
                        -- Calculate total work hours as the difference between check_in and check_out
                    CASE 
                        WHEN main_tbl.ac_sign_in IS NOT NULL AND main_tbl.ac_sign_out IS NOT NULL 
                        THEN (main_tbl.ac_sign_out - main_tbl.ac_sign_in)
                        ELSE 0.0
                    ...
                    {8}
                    --  ORDER BY main_tbl.dept_name, main_tbl.date, main_tbl.emp_name
                    ORDER BY main_tbl.dept_name, main_tbl.date, {12}
        ...
        data = {
            'model': "daily.attendance.sheet.wizard",
            'form': self.read()[0],
            'csr': data_res,
