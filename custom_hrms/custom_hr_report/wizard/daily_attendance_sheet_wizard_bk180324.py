...
class DailyAttendanceSheetWizard(models.TransientModel):
    ...
    def _set_domain_work_loc(self):
        if self.env.user.work_location_id:
            return [('is_work_loc', '=', True), ('state', '=', 'done'),
                    ('id', '=', self.env.user.work_location_id.id)]
        else:
            return [('is_work_loc', '=', True), ('state', '=', 'done')]
    ...
    def _onchange_employees(self):
        ...
        if self.work_location_id:
            domain += [('user_work_location_id', '=', self.work_location_id.id)]
    ...
    def daily_attendance_sheet_excel(self):
        ...
        if employee_id:
            ...
            sheet.merge_range(1, 0, 2, 9, "Daily Attendance Sheet", format0)
            ...
            sheet.merge_range(3, 4, 3, 9, 'Work Location: {0}'.format(employee_id.work_location_id.name), format1)
            ...
            sheet.write(7, 1, 'Check-in', format2)
            sheet.write(7, 2, 'Check-out', format2)
            sheet.write(7, 3, 'Overtime', format2)
            sheet.write(7, 4, 'Late-in', format2)
            sheet.write(7, 5, 'Early Out', format2)
            sheet.write(7, 6, 'Auto/Manual', format2)
            sheet.write(7, 7, 'Manual Reason', format2)
            sheet.write(7, 8, 'Status', format2)
            sheet.write(7, 9, 'Note', format2)
            ...
            for rec in data['csr']:
                ...
                sheet.write(row, col + 9, rec['note'], format5)
            ...
            sheet.merge_range(1, 0, 2, 14, "Daily Attendance Sheet", format0)
            ...
            for rec in data['csr']:
                ...
                sheet.write(row, col + 14, rec['note'], format5)
    ...
    def daily_attendance_sheet_sql(self):
        ...
        if work_location_id:
            work_loc_filter = "AND hre.work_location_id = %s" % work_location_id.id
            work_loc_filter2 = "AND hl.work_location_id = %s" % work_location_id.id
            work_location_name = work_location_id.name
        ...
        data_sql = """
                    ...
                    """.format(from_date, to_date, work_loc_filter, dept_filter, emp_filter, work_loc_filter2,
                               dept_filter2, emp_filter2, status_filter)
        ...
        if employee_id:
            ...
            emp_work_loc_name = employee_id.work_location_id.name
