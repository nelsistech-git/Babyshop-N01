...
class DateWiseLateReportWizard(models.TransientModel):
    _name = "date.wise.late.report.wizard"
    _description = "Date wise Late Report Wizard"

    file_data = fields.Binary('Date wise Late Report Wizard')
    ...
    category_ids = fields.Many2many('hr.employee.category', 'date_wise_late_employee_category_rel', 
    ...
    def date_wise_late_report_excel(self):
        ...
        # get data from sql
        data = self.date_wise_late_report_sql(from_date, to_date, department_id, user_work_location_id)

        from_date = datetime.strptime(str(from_date), '%Y-%m-%d').strftime('%d-%b-%Y')
        to_date = datetime.strptime(str(to_date), '%Y-%m-%d').strftime('%d-%b-%Y')

        file_name = "Date wise Late Report (%s - %s).xlsx" % (data['form']['from_date'], data['form']['to_date'])
        ...
        for line in data['csr']:
            for line2 in line:
                ...
                sheet.merge_range(3, 6, 3, 8, 'Tags: {0}'.format(','.join(self.category_ids.mapped('display_name'))) if self.sbu_unit_id else "Tags: No Tags Selected", format1)
                ...
                sheet.write(4, 4, 'Late Time', format2)
                sheet.write(4, 5, 'Actual Late Days', format2)
                sheet.write(4, 6, 'Deduction Absent', format2)
                sheet.write(4, 7, 'Real Absent Days', format2)
                sheet.write(4, 8, 'Total Absent Day', format2)
                ...
                for line3 in line[line2]:
                    ...
                    sheet.write(row, col + (new_col := new_col + 1), line3['emp_id_card'], format4)
                    sheet.write(row, col + (new_col := new_col + 1), line3['department_name'], format4)
                    sheet.write(row, col + (new_col := new_col + 1), line3['designation_name'], format4)
                    sheet.write(row, col + (new_col := new_col + 1), line3['late_days'], format5)
                    sheet.write(row, col + (new_col := new_col + 1), line3['total_late'], format5)
                    sheet.write(row, col + (new_col := new_col + 1), line3['deduct_abs'], format5)
                    sheet.write(row, col + (new_col := new_col + 1), line3['total_ab'], format5)
                    sheet.write(row, col + (new_col := new_col + 1), line3['total_ab_with_deduct'], format5)
        ...
        return {
            'name': 'Date wise Late Report',
            'type': 'ir.actions.act_url',
            'url': '/web/content?model=date.wise.late.report.wizard&field=file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'self',
        }

    def date_wise_late_report_pdf(self):
        from_date = self.from_date
        to_date = self.to_date
        department_id = self.department_id
        user_work_location_id = self.user_work_location_id
        ...
        return self.env.ref(
            'custom_hr_report.date_wise_late_report_tmpl').with_context(landscape=True).report_action(self, data=data)

    def date_wise_late_report_sql(self, from_date, to_date, department_id, user_work_location_id):
        # m = int(month)
        # y = int(year)
        ...
        #     late_days = ''
        #     if d['late_days']:
        #         late_days = ', '.join(map(str, sorted(list(map(int, d['late_days'].split(', '))))))
        #     vals = {
        ...
        data = {
            'model': "date.wise.late.report.wizard",
            'form': self.read()[0],
            'csr': final_data_list,
