...
class HrEmployee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        if employee:
            ...
            if employee[0]['birthday']:
                ...
                age = False
            if employee[0]['initial_employment_date']:
                diff = relativedelta(datetime.today(),
                                     employee[0]['initial_employment_date'])
                years = diff.years
                months = diff.months
                days = diff.days
            ...
            if employee:
                data = {
                    ...
                    'age': age,
                    'country_id': employee[0]['country_id'],
                    'mobile_phone': employee_obj.contact_no if employee_obj else ''
                }
    ...
    def employee_leave_trend(self):
        ...
        sql = """
                ...
                date_trunc('month', now()) - interval '6 month' and
                date_trunc('month', GREATEST(y , h.date_from)) <= 
                date_trunc('month', now()) and h.employee_id = %s """ %(emp_id)
        self.env.cr.execute(sql)
